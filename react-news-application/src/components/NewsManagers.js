...
const NewsManagers=props=>{
    ...
    return <Fragment>
    <NewsHeader  searchUpdateHandler={searchUpdate}  readListHandler={ReadListUpdate}  topHeadLineHandler={TopHeadlineupdate} categoryHandler={CategoryUpdate} />
   <div className="container my-3">
     <h2>{`${heading}${(newsType.id!=1 && newsType.id!=4)?newsType.result:``}`}</h2>
        <NewsDashboard news={news} isReadList={isReadList} />
   </div>
    </Fragment>
}
export default NewsManagers;